...
const firstName = location.state && location.state.firstName;
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const response = await axios.get('dreamlo.com/lb/9Aer_96Ftk6SjhkuZyjgkALyyXxqLcfU65gqW_Z48zUw/json');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingEntryIndex !== -1) {
        ...
        await axios.get(`dreamlo.com/lb/9Aer_96Ftk6SjhkuZyjgkALyyXxqLcfU65gqW_Z48zUw/add/${firstName}/${updatedScores[existingEntryIndex].score}`);
        setLeaderboardData(updatedScores); // Update the state
      } else {
        // If the name doesn't exist in the leaderboard, add a new entry
        await axios.get(`dreamlo.com/lb/9Aer_96Ftk6SjhkuZyjgkALyyXxqLcfU65gqW_Z48zUw/add/${firstName}/${score}`);
